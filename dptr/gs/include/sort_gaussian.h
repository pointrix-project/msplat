...
/**
 * @brief Launching CUDA kernel to compute key for Gaussians based on the tile index and depth.
 * 
 ...
 * @param[in] tiles               Number of tiles covered by 2D planar Gaussians on the image.
 * @return std::tuple<torch::Tensor, torch::Tensor>  <br>
 *         (1)<b>gaussian_key</b> Key of Gaussian.   <br>
 *         (2)<b>gaussian_idx</b> The gaussian index. If the same gaussian is in more than one tile, it will be repeated multiple times.
 */
 ...
 *         <b>tile_range</b>  Ranges of indices in idx_sorted for Gaussians in each tile.
 */
torch::Tensor
